...
export class Migration20260326082314 extends Migration {
    override async up(): Promise<void> {
        this.addSql(
            `create table "employee" ("id" uuid not null, "user_id" varchar(255) null, "first_name" varchar(255) not null, "last_name" varchar(255) not null, "email" varchar(255) null, "phone" varchar(255) null, "status" text not null, primary key ("id"));`,
        );
        this.addSql(`alter table "employee" add constraint "employee_user_id_unique" unique ("user_id");`);

        this.addSql(
            `create table "employee_permission_override" ("id" uuid not null default gen_random_uuid(), "permission_key" varchar(255) not null, "state" text not null, "employee_id" uuid not null, primary key ("id"));`,
        );

        this.addSql(
            `create table "employee_position_assignment" ("id" uuid not null default gen_random_uuid(), "position_key" varchar(255) not null, "assigned_at" timestamptz not null, "qualifications" jsonb not null default '[]', "employee_id" uuid not null, primary key ("id"));`,
        );
        ...
        this.addSql(
            `alter table "customer" add "customer_type" text not null, add "note" varchar(255) null, add "first_name" varchar(255) null, add "last_name" varchar(255) null, add "company_name" varchar(255) null, add "nip" varchar(255) null;`,
        );
        this.addSql(
            `alter table "customer" add constraint "customer_customer_type_check" check ("customer_type" in ('b2c', 'b2b', 'b2a'));`,
        );

        this.addSql(`alter table "customer_address" add "note" varchar(255) null;`);

        this.addSql(`alter table "customer_contact" drop constraint "customer_contact_type_check";`);
        this.addSql(
            `alter table "customer_contact" add "note" varchar(255) null, add "custom_label" varchar(255) null, add "history" jsonb not null default '[]';`,
        );
        this.addSql(

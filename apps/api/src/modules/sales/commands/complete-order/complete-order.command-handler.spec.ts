...
function createMocks(order: OrderAggregate | null = createDraftOrder()) {
    ...
    const handler = new CompleteOrderCommandHandler(orderRepo as OrderRepositoryPort, uow, eventBus as any);

    return { handler, orderRepo, uow, eventBus };
}

describe("CompleteOrderCommandHandler", () => {
    it("completes a placed order and persists it", async () => {
        const { handler, orderRepo, uow, eventBus } = createMocks();

        await handler.execute(new CompleteOrderCommand({ orderId: randomUUID() }));
    ...
    it("throws OrderNotFoundError when order does not exist", async () => {
        ...
        await expect(handler.execute(new CompleteOrderCommand({ orderId: randomUUID() }))).rejects.toThrow(
            OrderNotFoundError,
        );
    });
});

...
export class Migration20260328130000 extends Migration {
    override async up(): Promise<void> {
        this.addSql(
            `create table "journey" ("id" uuid not null, "route_id" uuid not null, "route_name" varchar(255) not null, "status" text not null, "scheduled_date" varchar(255) not null, "vehicle_ids" text[] not null default '{}', "representative_ids" text[] not null default '{}', "stops" jsonb not null default '[]', "loading_deadline" varchar(255) null, primary key ("id"));`,
        );
        ...
        this.addSql(
            `alter table "journey" add constraint "journey_status_check" check ("status" in ('PLANNED', 'AWAITING_LOADING', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'));`,
        );

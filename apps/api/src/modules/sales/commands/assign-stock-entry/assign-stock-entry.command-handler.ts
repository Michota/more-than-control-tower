...
export class AssignStockEntryCommandHandler implements ICommandHandler<AssignStockEntryCommand> {
    ...
    async execute(cmd: AssignStockEntryCommand): Promise<void> {
        ...
        // Verify the stock entry's good matches the order line's good
        const line = order
            .getOrderLines()
            .getLines()
            .get(cmd.productId as any);
        if (line?.goodId && stockEntry.goodId !== line.goodId) {
            throw new StockEntryGoodMismatchError(cmd.stockEntryId);
        }

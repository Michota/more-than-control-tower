...
import { AssignOrderToStopCommand } from "./assign-order-to-stop.command.js";

@CommandHandler(AssignOrderToStopCommand)
export class AssignOrderToStopCommandHandler implements ICommandHandler<AssignOrderToStopCommand> {
    constructor(
    ...
    async execute(cmd: AssignOrderToStopCommand): Promise<void> {
        const journey = await this.journeyRepo.findOneById(cmd.journeyId);
        if (!journey) {
            throw new JourneyNotFoundError(cmd.journeyId);
        }

        journey.assignOrderToStop(cmd.customerId, cmd.orderId);

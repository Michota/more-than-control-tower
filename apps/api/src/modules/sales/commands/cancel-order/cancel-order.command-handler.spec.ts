...
import { CancelOrderCommandHandler } from "./cancel-order.command-handler.js";
...
function createMocks(order: OrderAggregate | null = createDraftOrder()) {
    ...
    const handler = new CancelOrderCommandHandler(orderRepo as OrderRepositoryPort, uow, eventBus as any);

    return { handler, orderRepo, uow, eventBus };
}

describe("CancelOrderCommandHandler", () => {
    it("cancels an order and persists it", async () => {
        const { handler, orderRepo, uow, eventBus } = createMocks();

        await handler.execute(new CancelOrderCommand({ orderId: randomUUID() }));
    ...
    it("throws OrderNotFoundError when order does not exist", async () => {
        ...
        await expect(handler.execute(new CancelOrderCommand({ orderId: randomUUID() }))).rejects.toThrow(
            OrderNotFoundError,
        );
    });
});
